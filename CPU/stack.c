...
void stack_dump_printf(char *str) {
#ifdef DEBUG
...
int Stack_Size(stack * Stack){
	...
	int Size;
	for(Size = 0; currentNode != NULL; Size++, currentNode = currentNode->prev);
	return Size;
}

int Stack_Dump(stack* Stack) {
	//int size = Stack_Size(Stack);
	stack_dump_printf("DUMP Stack ok\n");
	int i;
	stack* currentNode = Stack;
	for(i = 0; currentNode != NULL && currentNode->prev != NULL; i++, currentNode = currentNode->prev){
		//ASSERT(currentNode != NULL);
		printf("%d : %lg \n", i, currentNode->Value);
	}
	stack_dump_printf("DUMP complite\n");
	//ASSERT_STACK_OK(Stack);
	return 0;
}

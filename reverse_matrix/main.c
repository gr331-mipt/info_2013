...
mytype **matrix_initialize(const int n) {
	...
	int i;
	for(i = 0; i < n; i++)
		matrix[i] = (mytype*) calloc(2 * n, sizeof(mytype));
	return matrix;
}
...
void matrix_destroy(mytype **matrix, int n1, int n2) {
	int i;
	for(i = 0; i < n1; i++)
		free(matrix[i]);
	free(matrix);
...
int main() {
	...
	if(!matrix_identity(a, n, 2 * n))
		printf("NO");
	else
		matrix_invertible(a, n);
	matrix_destroy(a, n, 2 * n);
	return 0;
}
